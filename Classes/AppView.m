...
- (id)initWithFrame:(CGRect)frame {
    ...
    if ((self = [super initWithFrame:frame])) {
		...
		gameController = [[AppController alloc] init];
...
- (void)mainGameLoop {
    ...
    while(true) {
        ...
        if(_FPSCounter > 0.25f) {
            ...
            [gameManager setFramesPerSecond:_fps];
        }
...
- (void)drawView {
	...
	// Set the current EAGLContext and bind to the framebuffer.  This will direct all WaterPipe commands to the
