...
- (Image*)getSubImageAtPoint:(CGPoint)aPoint subImageWidth:(GLuint)aImageWidth subImageHeight:(GLuint)aImageHeight scale:(float)aScale {
	...
	[subImage setColourFilterRed:colorFilter.red green:colorFilter.green blue:colorFilter.blue alpha:colorFilter.alpha];
...
- (void)renderAt:(CGPoint)point texCoords:(Quad2f*)tc quadVertices:(Quad2f*)qv {
	...
	//update location of rect
	rect = CGRectMake(point.x, point.y, textureWidth, textureHeight);
	//CGRectOffset(rect, point.x, point.y);
	...
	glColor4f(colorFilter.red, colorFilter.green, colorFilter.blue, colorFilter.alpha * [gameManager globalAlpha]);
	...
	if([texture name] != [gameManager currentlyBoundTexture]) {
		[gameManager setCurrentlyBoundTexture:[texture name]];
...
- (void)setColourFilterRed:(GLfloat)aRed green:(GLfloat)aGreen blue:(GLfloat)aBlue alpha:(GLfloat)alpha {
	colorFilter.red = aRed;
	colorFilter.green = aGreen;
	colorFilter.blue = aBlue;
	colorFilter.alpha = alpha;
}


- (void)setAlpha:(GLfloat)aAlpha {
	colorFilter.alpha = aAlpha;
}

- (GLfloat) getAlpha{
	return colorFilter.alpha;
}
