						  ...
						  blendSFactor:(GLenum)inBlendSFactor
{
	...
	if (self != nil) {
		...
		// By default, smooth entry is off. Only problematic animations that have a cluster in the beginning need to use this
		easeIn = NO;
		easingIn = NO;
		...
		// Mutable max particles is set to max
		mutableMaxParticles = maxParticles;
		particleLimiter = maxParticles;
	}
	return self;
}
...
- (BOOL)addParticle {
	...
	if(particleCount >= mutableMaxParticles)
		return NO;
...
- (void)update:(GLfloat)delta {
	...
	//detect when the PE goes from active NO to active YES
	if (wasActive != active && active == YES && easeIn){
		easingIn = YES;
	
		mutableMaxParticles = maxParticles * 0.2;
		if (mutableMaxParticles > particleLimiter){
			mutableMaxParticles = particleLimiter;
		}
	}
	wasActive = active;
	
	//if PE has easingIn set to YES, have animation start slowly to prevent initial clusters
	if (easeIn && easingIn){
		mutableMaxParticles += (maxParticles / 20);
		...
		if (mutableMaxParticles >= particleLimiter){
			easingIn = NO; //easing in is complete
			mutableMaxParticles = maxParticles;
		}
	}
	...
	if(active && emissionRate) {
		...
		emitCounter += delta;
		while(particleCount < mutableMaxParticles && emitCounter > rate) {
			[self addParticle];
			emitCounter -= rate;
		}
	...
	while(particleIndex < particleCount) {
		...
		if(currentParticle->timeToLive > 0) {
			...
			//NSLog(@"recorded p size = %f", vertices[particleIndex].size);
...
- (void)renderParticles {
	...
	if(texture && [gameManager currentlyBoundTexture] != [[texture texture] name]) {
		glBindTexture(GL_TEXTURE_2D, [[texture texture] name]);
		[gameManager setCurrentlyBoundTexture:[[texture texture] name]];
	}
	
	// Enable and configure blending
	glEnable(GL_BLEND);

	// Change the S factor of the blend to get different effects
	glBlendFunc(blendSFactor, GL_ONE);
	//GL_DST_COLOR = Overlay effect 
	//GL_ONE_MINUS_DST_COLOR = solid with background that fades to white
	//GL_SRC_ALPHA = solid with background that fades to black
